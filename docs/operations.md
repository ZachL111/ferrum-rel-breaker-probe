...
`ferrum-rel-breaker-probe` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for breaker scenarios with node-edge fixtures, cycle and reachability reports, and no network dependency.
...
The score is deterministic. The current threshold is 160. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 2.
